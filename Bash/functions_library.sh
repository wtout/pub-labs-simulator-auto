...
function get_avmlist() {
	local avmlist
	[[ "$(echo "${@}" | grep -Ew '\-\-avmlist')" != "" ]] && avmlist="$(echo "${@}" | awk -F 'avmlist ' '{print $NF}' | cut -d'-' -f1 | xargs)"
	echo "${avmlist}"
}
...
function check_arguments() {
	...
	if [[ "$(echo "${@}" | grep -Ew '\-\-avmlist')" != "" ]]
	then
		[[ $(wc -w <<< "$(get_avmlist "${@}")") -ge 2 ]] && printf "\nAVM IDs must be separated by a comma. Aborting!\n\n" && exit 1
		[[ $(wc -w <<< "$(get_avmlist "${@}")") -lt 1 ]] && printf "\nYou need to specify at least one AVM ID. Aborting!\n\n" && exit 1
	fi
}
...
function run_playbook() {
	...
	then
		...
		local EVARGS
		if [[ "$(echo "${ANSIBLE_CMD_ARGS}" | grep -w '\-\-tags devsim')" == "" ]]
		then
			EVARGS="{SVCFILE: '${SVCVAULT}', $(basename "${0}" | sed -e 's/\(.*\)\.sh/\1/'): true}"
		else
			if  [[ "$(echo "${0}" | grep 'check')" == "" ]]
			then
				EVARGS="{SVCFILE: '${SVCVAULT}', AVMLIST: '${ALIST}', $(basename "${0}" | sed -e 's/\(.*\)\.sh/\1/'): true}"
			else
				EVARGS="{SVCFILE: '${SVCVAULT}', $(basename "${0}" | sed -e 's/\(.*\)\.sh/\1/'): true}"
			fi
		fi
